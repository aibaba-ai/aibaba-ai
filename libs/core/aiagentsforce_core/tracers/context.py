...
from aiagentsforce_core.tracers.schemas import TracerSessionV1

if TYPE_CHECKING:
    from langsmith import Client as LangSmithClient

    from aiagentsforce_core.callbacks.base import BaseCallbackHandler, Callbacks
    from aiagentsforce_core.callbacks.manager import AsyncCallbackManager, CallbackManager

# for backwards partial compatibility if this is imported by users but unused
tracing_callback_var: Any = None
tracing_v2_callback_var: ContextVar[Optional[AI Agents ForceTracer]] = ContextVar(
...
def tracing_v2_enabled(
    ...
    *,
...
) -> Generator[AI Agents ForceTracer, None, None]:
    """Instruct AI Agents Force to log all runs in context to LangSmith.
    ...
    Yields:
        AI Agents ForceTracer: The AI Agents Force tracer.

    Example:
        >>> with tracing_v2_enabled():
        ...     # AI Agents Force code will automatically be traced
...
def _get_trace_callbacks(
    ...
    if _tracing_v2_is_enabled():
        ...
        tracer = tracing_v2_callback_var.get() or AI Agents ForceTracer(
            project_name=project_name_,
            example_id=example_id,
        )
        if callback_manager is None:
            from aiagentsforce_core.callbacks.base import Callbacks
            ...
            if not any(
                ...
                # If it already has a AI Agents ForceTracer, we don't need to add another one.
...
def register_configure_hook(
    ...
    from aiagentsforce_core.callbacks.base import BaseCallbackHandler
