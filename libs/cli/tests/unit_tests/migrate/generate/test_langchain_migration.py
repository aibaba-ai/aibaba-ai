...
def test_create_json_agent_migration() -> None:
    """Test the migration of create_json_agent from langchain to aibaba_ai_community."""
    with sup1():
        with sup2():
            raw_migrations = generate_simplified_migrations(
                from_package="langchain", to_package="aibaba_ai_community"
            )
            ...
            assert json_agent_migrations == [
                (
                    ...
                    "aibaba_ai_community.agent_toolkits.create_json_agent",
                ),
                (
