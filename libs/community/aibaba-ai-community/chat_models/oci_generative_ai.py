...
from aibaba-ai-core.messages.tool import ToolCallChunk
...
def _convert_oci_tool_call_to_langchain(tool_call: Any) -> ToolCall:
    """Convert a OCI GenAI tool call into aibaba-ai-core.messages.ToolCall"""
    _id = uuid.uuid4().hex[:]
...
class MetaProvider(Provider):
    ...
    def messages_to_oci_params(
        ...
        """Convert Aibaba AI messages to OCI chat parameters.

        Args:
            messages: List of Aibaba AI BaseMessage objects
...
class ChatOCIGenAI(BaseChatModel, OCIGenAIBase):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        Args:
            messages: list of Aibaba AI messages
            stop: Optional list of stop words to use.

        Returns:
            Aibaba AI ChatResult
