...
from aibaba-ai-core.utils.pydantic import get_fields
...
class DocArrayRetriever(BaseRetriever):
    ...
    def _similarity_search(self, query_emb: np.ndarray) -> List[Document]:
        ...
        results = [self._docarray_to_aiagentforce_doc(doc) for doc in docs]
        return results
    ...
    def _mmr_search(self, query_emb: np.ndarray) -> List[Document]:
        ...
        results = [self._docarray_to_aiagentforce_doc(docs[idx]) for idx in mmr_selected]
        return results

    def _docarray_to_aiagentforce_doc(self, doc: Union[Dict[str, Any], Any]) -> Document:
        """
