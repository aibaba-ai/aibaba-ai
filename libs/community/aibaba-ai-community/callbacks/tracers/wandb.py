...
from aibaba-ai-core._api import warn_deprecated
from aibaba-ai-core.output_parsers.pydantic import PydanticBaseModel
...
class WandbTracer(BaseTracer):
    ...
    This will ensure that all Aibaba AI activity is logged to W&B.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        To use W&B to monitor all Aibaba AI activity, add this tracer like any other
        Aibaba AI callback:
        ```
        ...
        warn_deprecated(
            ...
            message=(
                ...
                "and tracing your Aibaba AI callables."
    ...
    def _ensure_run(self, should_print_url: bool = False) -> None:
        ...
        if self._wandb.run is not None:
            if should_print_url:
                ...
                self._wandb.termlog(
                    f"Streaming Aibaba AI activity to W&B at {run_url}\n"
    ...
    def _log_trace_from_run(self, run: Run) -> None:
        """Logs a Aibaba AI Run to W*B as a W&B Trace."""
        self._ensure_run()
