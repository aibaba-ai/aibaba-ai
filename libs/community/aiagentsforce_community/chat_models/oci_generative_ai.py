...
from aiagentsforce_core.messages.tool import ToolCallChunk
...
def _convert_oci_tool_call_to_langchain(tool_call: Any) -> ToolCall:
    """Convert a OCI GenAI tool call into aiagentsforce_core.messages.ToolCall"""
    _id = uuid.uuid4().hex[:]
...
class MetaProvider(Provider):
    ...
    def messages_to_oci_params(
        ...
        """Convert AI Agents Force messages to OCI chat parameters.

        Args:
            messages: List of AI Agents Force BaseMessage objects
...
class ChatOCIGenAI(BaseChatModel, OCIGenAIBase):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        Args:
            messages: list of AI Agents Force messages
            stop: Optional list of stop words to use.

        Returns:
            AI Agents Force ChatResult
