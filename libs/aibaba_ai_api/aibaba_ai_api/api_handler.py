...
from aiagentsforceapi._pydantic import _create_root_model
from aiagentsforceapi.callbacks import AsyncEventAggregatorCallback, CallbackEventDict
from aiagentsforceapi.lzstring import LZString
from aiagentsforceapi.playground import serve_playground
from aiagentsforceapi.schema import (
...
from aiagentsforceapi.serialization import Serializer, WellKnownLCSerializer
from aiagentsforceapi.validation import (
    BatchBaseResponse,
...
def _is_hosted() -> bool:
    return os.environ.get("HOSTED_aiagentsforceapi_ENABLED", "false").lower() == "true"
...
def _update_config_with_defaults(
    ...
    *,
    ...
    metadata = {
        ...
        "__aiagentsforceapi_version": __version__,
    }

    if endpoint:
        metadata["__aiagentsforceapi_endpoint"] = endpoint

    if _is_hosted():
        hosted_metadata = {
            "__aiagentsforceapi_hosted_git_ref": os.environ.get(
                "HOSTED_aiagentsforceapi_GIT_REF", ""
            ),
            "__aiagentsforceapi_hosted_repo_subdirectory_path": os.environ.get(
                "HOSTED_aiagentsforceapi_GIT_REPO_PATH", ""
            ),
            "__aiagentsforceapi_hosted_repo_url": os.environ.get(
                "HOSTED_aiagentsforceapi_GIT_REPO", ""
            ),
            "__aiagentsforceapi_hosted_revision_id": os.environ.get(
                "HOSTED_aiagentsforceapi_REVISION_ID", ""
            ),
            "__aiagentsforceapi_hosted_is_hosted": "true",
        }
    ...
    async def create_feedback(
        ...
        if not tracing_is_enabled() or not self._enable_feedback_endpoint:
            raise HTTPException(
                ...
                + "enabled on your aiagentsforceapi server.\n"
    ...
    async def create_public_trace_link(
        ...
        if not tracing_is_enabled() or not self._enable_public_trace_link_endpoint:
            raise HTTPException(
                ...
                + "LangSmith is enabled on your aiagentsforceapi server.\n"
