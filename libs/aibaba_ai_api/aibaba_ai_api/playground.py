...
def _get_mimetype(path: str) -> str:
    ...
    Resolves issue: https://github.com/aibaba-aiaiagentsforceapi/issues/245
        ...
        with open(local_file_path, encoding="utf-8") as f:
            ...
            if mime_type in ("text/html", "text/css", "application/javascript"):
                response = PlaygroundTemplate(f.read()).substitute(
                    aiagentsforceapi_BASE_URL=base_url[1:]
                    if base_url.startswith("/")
                    else base_url,
                    aiagentsforceapi_CONFIG_SCHEMA=json.dumps(
                        runnable.config_schema(include=config_keys).model_json_schema()
                    ),
                    aiagentsforceapi_INPUT_SCHEMA=json.dumps(input_schema.model_json_schema()),
                    aiagentsforceapi_OUTPUT_SCHEMA=json.dumps(
                        output_schema.model_json_schema()
                    ),
                    aiagentsforceapi_FEEDBACK_ENABLED=json.dumps(
                        "true" if feedback_enabled else "false"
                    ),
                    aiagentsforceapi_PUBLIC_TRACE_LINK_ENABLED=json.dumps(
