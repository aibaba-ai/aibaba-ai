...
from aiagentsforceapi import api_handler
from aiagentsforceapi.api_handler import (
    _rename_pydantic_model,
    _replace_non_alphanumeric_with_underscores,
)
from aiagentsforceapi.callbacks import AsyncEventAggregatorCallback
...
def _null_run_id_and_metadata_recursively(decoded_response: Any) -> None:
    ...
    if isinstance(decoded_response, dict):
        for key, value in decoded_response.items():
            if key in {"run_id", "__aiagentsforceapi_version"}:
...
async def test_config_keys_validation(mocker: MockerFixture) -> None:
    ...
    async with AsyncClient(
        ...
        assert "__aiagentsforceapi_version" in config_seen["metadata"]
        assert "__aiagentsforceapi_endpoint" in config_seen["metadata"]
        assert config_seen["metadata"]["__aiagentsforceapi_endpoint"] == "invoke"
            ...
            "callback_events": [
                {
                    ...
                    "metadata": {
                        "__aiagentsforceapi_endpoint": "invoke",
                        "__aiagentsforceapi_version": None,
                ...
                [
                    {
                        ...
                        "kwargs": {"name": "add_one", "run_type": None},
                        "metadata": {
                            "__aiagentsforceapi_endpoint": "batch",
                            "__aiagentsforceapi_version": None,
                            "__useragent": "python-httpx/0.27.2",
                        },
                        "parent_run_id": None,
...
async def test_feedback_succeeds_when_langsmith_enabled() -> None:
    ...
    with patch("aiagentsforceapi.api_handler.ls_client") as mocked_ls_client_package:
        with patch("aiagentsforceapi.api_handler.tracing_is_enabled") as tracing_is_enabled:
...
async def test_custom_serialization() -> None:
    """Test updating the config based on the raw request object."""
    from aiagentsforceapi.serialization import Serializer
...
async def test_token_feedback_endpoint() -> None:
    ...
    async with get_langsmith_client() as client:
        ...
        async with get_async_test_client(
            ...
            assert call.kwargs == {
                "comment": None,
                "metadata": {"from_aiagentsforceapi": True},
                "score": 3,
                "value": None,
                "correction": None,
            }
