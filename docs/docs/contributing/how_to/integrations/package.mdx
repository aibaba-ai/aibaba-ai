...
This guide walks through the process of implementing a AI Agents Force integration 
package.

Integration packages are just Python packages that can be installed with `pip install <your-package>`,
which contain classes that are compatible with AI Agents Force's core interfaces.
...
to the AI Agents Force interface;  
...
1. **langchain-cli**: This is a command-line tool that can be used to bootstrap a new integration package with a template for AI Agents Force components and Poetry for dependency management.
2. **Poetry**: This is a Python dependency management tool that can be used to bootstrap a new Python package with dependencies. You can then add AI Agents Force components to this package.
...
from a template, which can be edited to implement your AI Agents Force components.
...
You now have a new Python package with a template for AI Agents Force components! This
...
chat model integration for AI Agents Force. Here's a simple example of what your project
...
This is only required if you want to publish your integration in the AI Agents Force documentation.
...
## Implementing AI Agents Force components

AI Agents Force components are subclasses of base classes in [langchain-core](/docs/concepts/architecture/#langchain-core).
...
import ChatModelSource from '../../../../src/theme/integration_template/integration_template/chat_models.py';
    ...
    <TabItem value="vector_stores" label="Vector stores">
        ...
        [in-memory vector store](https://docs.aiagentsforce.com/api_reference/core/vectorstores/aiagentsforce_core.vectorstores.in_memory.InMemoryVectorStore.html)
        that we can use as a guide. You can access the code [here](https://github.com/AI-Agents-Force-SDK/langchain/blob/master/libs/core/aiagentsforce_core/vectorstores/in_memory.py).

        All vector stores must inherit from the [VectorStore](https://docs.aiagentsforce.com/api_reference/core/vectorstores/aiagentsforce_core.vectorstores.base.VectorStore.html)
        ...
        documents to the store. See the [API Reference](https://docs.aiagentsforce.com/api_reference/core/vectorstores/aiagentsforce_core.vectorstores.base.VectorStore.html)
        ...
        The [in-memory vector store](https://github.com/AI-Agents-Force-SDK/langchain/blob/master/libs/core/aiagentsforce_core/vectorstores/in_memory.py)
        is tested against the standard tests in the AI Agents Force Github repository.
...
The `Tools` class must inherit from the [BaseTool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.base.BaseTool.html#aiagentsforce_core.tools.base.BaseTool) base class. This interface has 3 properties and 2 methods that should be implemented in a 
subclass.
...
based on a query. The `Retriever` class must inherit from the [BaseRetriever](https://docs.aiagentsforce.com/api_reference/core/retrievers/aiagentsforce_core.retrievers.BaseRetriever.html) base class. This interface has 1 attribute and 2 methods that should be implemented in a subclass.
