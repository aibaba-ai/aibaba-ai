...
```python
from aiagentsforce_community.chat_models import ChatParrotLink
from aiagentsforce_community.llms import ParrotLinkLLM
from aiagentsforce_community.vectorstores import ParrotLinkVectorStore
```
...
Let's say we wanted to implement a chat model for Parrot Link AI. We would create a new file in `libs/community/aiagentsforce_community/chat_models/parrot_link.py` with the following code:
