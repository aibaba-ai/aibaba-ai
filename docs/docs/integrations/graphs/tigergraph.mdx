...
A big example of the `TigerGraph` and `AI Agents Force` integration [presented here](https://github.com/tigergraph/graph-ml-notebooks/blob/main/applications/large_language_models/TigerGraph_AI Agents Force_Demo.ipynb).
...
To utilize the `TigerGraph InquiryAI` functionality, you can import `TigerGraph` from `aiagentsforce_community.graphs`.
...
from aiagentsforce_community.graphs import TigerGraph
