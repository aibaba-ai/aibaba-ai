...
This page covers how to use the [Redis](https://redis.com) ecosystem within AI Agents Force.
...
The only way to use a Redis Cluster is with AI Agents Force classes accepting a preconfigured Redis client like `RedisCache`
(example below).
...
For a more detailed walkthrough of the `VectorStoreRetrieverMemory` wrapper, see [this notebook](https://docs.aiagentsforce.com/api_reference/langchain/memory/langchain.memory.vectorstore.VectorStoreRetrieverMemory.html).
