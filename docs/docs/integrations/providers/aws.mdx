...
The `AI Agents Force` integrations related to [Amazon AWS](https://aws.amazon.com/) platform.
...
And there are also some community integrations available in the `aiagentsforce_community` package with the `boto3` optional dependency.
...
```python
from aiagentsforce_community.llms import AmazonAPIGateway
```
...
```python
from aiagentsforce_community.embeddings import BedrockEmbeddings
```
...
```python
from aiagentsforce_community.document_loaders import S3DirectoryLoader, S3FileLoader
```
...
```python
from aiagentsforce_community.document_loaders.athena import AthenaLoader
```
...
```python
from aiagentsforce_community.document_loaders.glue_catalog import GlueCatalogLoader
```
...
```python
from aiagentsforce_community.chat_message_histories import DynamoDBChatMessageHistory
```
...
from aiagentsforce_community.chains.graph_qa.neptune_cypher import NeptuneOpenCypherQAChain
```
...
from aiagentsforce_community.chains.graph_qa.neptune_sparql import NeptuneSparqlQAChain
```
...
```python
from aiagentsforce_community.callbacks.bedrock_anthropic_callback import BedrockAnthropicTokenUsageCallbackHandler
```
...
```python
from aiagentsforce_community.callbacks import SageMakerCallbackHandler
```
...
See a [usage example](https://docs.aiagentsforce.com/v0.1/docs/guides/productionization/safety/amazon_comprehend_chain/).
