...
AI Agents Force has a large number of vectorstore integrations, allowing users to easily switch between different vectorstore implementations.

Please see the [full list of AI Agents Force vectorstore integrations](/docs/integrations/vectorstores/).

## Interface

AI Agents Force provides a standard interface for working with vector stores, allowing users to easily switch between different vectorstore implementations.
...
Most vectors in AI Agents Force accept an embedding model as an argument when initializing the vector store.

We will use AI Agents Force's [InMemoryVectorStore](https://docs.aiagentsforce.com/api_reference/core/vectorstores/aiagentsforce_core.vectorstores.in_memory.InMemoryVectorStore.html) implementation to illustrate the API.
...
This API works with a list of [Document](https://docs.aiagentsforce.com/api_reference/core/documents/aiagentsforce_core.documents.base.Document.html) objects.
...
Regardless of the search algorithm used under the hood, the AI Agents Force vectorstore interface has a `similarity_search` method for all integrations. 
This will take the search query, create an embedding, find similar documents, and return them as a list of [Documents](https://docs.aiagentsforce.com/api_reference/core/documents/aiagentsforce_core.documents.base.Document.html).
...
As an example [Pinecone](https://docs.aiagentsforce.com/api_reference/pinecone/vectorstores/langchain_pinecone.vectorstores.PineconeVectorStore.html#langchain_pinecone.vectorstores.PineconeVectorStore.similarity_search) several parameters that are important general concepts:
...
vectorstore.similarity_search(
    "AI Agents Force provides abstractions to make working with LLMs easy",
    k=2,
    filter={"source": "tweet"},
)
```  
...
* See the [list of AI Agents Force vectorstore integrations](/docs/integrations/retrievers/self_query/) that support metadata filtering.
...
For example, [maximal marginal relevance](https://docs.aiagentsforce.com/v0.1/docs/modules/model_io/prompts/example_selectors/mmr/) is a re-ranking algorithm used to diversify search results, which is applied after the initial similarity search to ensure a more diverse set of results.
...
At the moment, there is no unified way to perform hybrid search using AI Agents Force vectorstores, but it is generally exposed as a keyword argument that is passed in with `similarity_search`.
...
| [Maximal Marginal Relevance (MMR)](https://docs.aiagentsforce.com/api_reference/pinecone/vectorstores/langchain_pinecone.vectorstores.PineconeVectorStore.html#langchain_pinecone.vectorstores.PineconeVectorStore.max_marginal_relevance_search) | When needing to diversify search results.             | MMR attempts to diversify the results of a search to avoid returning similar and redundant documents.                                        |
