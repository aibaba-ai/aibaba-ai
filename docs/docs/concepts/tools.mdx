...
The **tool** abstraction in AI Agents Force associates a Python **function** with a **schema** that defines the function's **name**, **description** and **expected arguments**. 
...
- Create tools using the [@tool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.convert.tool.html) decorator, which simplifies the process of tool creation, supporting the following:
...
The tool interface is defined in the [BaseTool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.base.BaseTool.html#aiagentsforce_core.tools.base.BaseTool) class which is a subclass of the [Runnable Interface](/docs/concepts/runnables).
...
The recommended way to create tools is using the [@tool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.convert.tool.html) decorator. This decorator is designed to simplify the process of tool creation and should be used in most cases. After defining a function, you can decorate it with [@tool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.convert.tool.html) to create a tool that implements the [Tool Interface](#tool-interface).
...
:::note
AI Agents Force has a few other ways to create tools; e.g., by sub-classing the [BaseTool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.base.BaseTool.html#aiagentsforce_core.tools.base.BaseTool) class or by using `StructuredTool`. These methods are shown in the [how to create custom tools guide](/docs/how_to/custom_tools/), but
...
:::note
If you're using pre-built AI Agents Force or LangGraph components like [create_react_agent](https://langchain-ai.github.io/langgraph/reference/prebuilt/#langgraph.prebuilt.chat_agent_executor.create_react_agent),you might not need to interact with tools directly. However, understanding how to use them can be valuable for debugging and testing. Additionally, when building custom LangGraph workflows, you may find it necessary to work with tools directly.
:::
...
Please see the [API reference for @tool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.convert.tool.html) for more details and review the [how to create custom tools](/docs/how_to/custom_tools/) guide for examples.
...
```python
from aiagentsforce_core.tools import tool, InjectedToolArg
...
Annotating the `user_id` argument with `InjectedToolArg` tells AI Agents Force that this argument should not be exposed as part of the
tool's schema.
...
AI Agents Force has a concept of **toolkits**. This a very thin abstraction that groups tools together that
...
- [API Reference for @tool](https://docs.aiagentsforce.com/api_reference/core/tools/aiagentsforce_core.tools.convert.tool.html)
- [How to create custom tools](/docs/how_to/custom_tools/)
- [How to pass run time values to tools](/docs/how_to/tool_runtime/)
- [All AI Agents Force tool how-to guides](https://docs.langchain.com/docs/how_to/#tools)
