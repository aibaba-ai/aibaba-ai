...
The Runnable interface is the foundation for working with AI Agents Force components, and it's implemented across many of them, such as [language models](/docs/concepts/chat_models), [output parsers](/docs/concepts/output_parsers), [retrievers](/docs/concepts/retrievers), [compiled LangGraph graphs](
https://langchain-ai.github.io/langgraph/concepts/low_level/#compiling-your-graph) and more.

This guide covers the main concepts and methods of the Runnable interface, which allows developers to interact with various AI Agents Force components in a consistent and predictable manner.

:::info Related Resources
* The ["Runnable" Interface API Reference](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.base.Runnable.html#aiagentsforce_core.runnables.base.Runnable) provides a detailed overview of the Runnable interface and its methods.
* A list of built-in `Runnables` can be found in the [AI Agents Force Core API Reference](https://docs.aiagentsforce.com/api_reference/core/runnables.html). Many of these Runnables are useful when composing custom "chains" in AI Agents Force using the [AI Agents Force Expression Language (LCEL)](/docs/concepts/lcel).
:::
...
* Composed: Multiple Runnables can be composed to work together using [the AI Agents Force Expression Language (LCEL)](/docs/concepts/lcel) to create complex pipelines.
...
AI Agents Force Runnables offer a built-in `batch` (and `batch_as_completed`) API that allow you to process multiple inputs in parallel.
...
Please refer to the [Async Programming with AI Agents Force](/docs/concepts/async) guide for more details.
...
1. sync [stream](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.base.Runnable.html#aiagentsforce_core.runnables.base.Runnable.stream) and async [astream](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.base.Runnable.html#aiagentsforce_core.runnables.base.Runnable.astream): yields the output a Runnable as it is generated.
...
Please refer to the [Streaming Conceptual Guide](/docs/concepts/streaming) for more details on how to stream in AI Agents Force.
...
These APIs are mostly used internally for unit-testing and by [aiagentsforceapi](/docs/concepts/architecture#aiagentsforceapi) which uses the APIs for input validation and generation of [OpenAPI documentation](https://www.openapis.org/).
...
`RunnableConfig` ([API Reference](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.config.RunnableConfig.html#RunnableConfig)). This argument is a dictionary that contains configuration for the Runnable that will be used
...
1. Declaratively using [AI Agents Force Expression Language (LCEL)](/docs/concepts/lcel):
...
AI Agents Force will try to propagate `RunnableConfig` automatically for both of the patterns. 

For handling the second pattern, AI Agents Force relies on Python's [contextvars](https://docs.python.org/3/library/contextvars.html).
...
:::note Related
* [How-to trace with AI Agents Force](https://docs.smith.langchain.com/how_to_guides/tracing/trace_with_langchain)
:::
...
It is used for a similar purpose in [RunnableWithMessageHistory](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.history.RunnableWithMessageHistory.html#aiagentsforce_core.runnables.history.RunnableWithMessageHistory) to specify either
...
Please read the [Callbacks Conceptual Guide](/docs/concepts/callbacks) for more information on how to use callbacks in AI Agents Force.
...
useful if using [AI Agents Force Expression Language (LCEL)](/docs/concepts/lcel) to compose
...
It helps with configuration of large "chains" created using the [AI Agents Force Expression Language (LCEL)](/docs/concepts/lcel)
and is leveraged by [aiagentsforceapi](/docs/concepts/architecture#aiagentsforceapi) for deployed Runnables.
:::
