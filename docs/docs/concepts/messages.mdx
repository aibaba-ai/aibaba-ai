...
AI Agents Force provides a unified message format that can be used across chat models, allowing users to work with different chat models without worrying about the specific details of the message format used by each model provider.
...
## AI Agents Force Messages

AI Agents Force provides a unified message format that can be used across all chat models, allowing users to work with different chat models without worrying about the specific details of the message format used by each model provider.

AI Agents Force messages are Python objects that subclass from a [BaseMessage](https://docs.aiagentsforce.com/api_reference/core/messages/aiagentsforce_core.messages.base.BaseMessage.html).
...
You can find more information about **messages** in the [API Reference](https://docs.aiagentsforce.com/api_reference/core/messages.html).
...
:::tip
When invoking a chat model with a string as input, AI Agents Force will automatically convert the string into a `HumanMessage` object. This is mostly useful for quick testing.
...
An `AIMessage` has the following attributes. The attributes which are **standardized** are the ones that AI Agents Force attempts to standardize across different chat model providers. **raw** fields are specific to the model provider and may vary.
...
| `usage_metadata`     | Standardized     | Usage metadata for a message, such as [token counts](/docs/concepts/tokens). See [Usage Metadata API Reference](https://docs.aiagentsforce.com/api_reference/core/messages/aiagentsforce_core.messages.ai.UsageMetadata.html). |
...
`AIMessageChunk` follows nearly the same structure as `AIMessage`, but uses a different [ToolCallChunk](https://docs.aiagentsforce.com/api_reference/core/messages/aiagentsforce_core.messages.tool.ToolCallChunk.html#aiagentsforce_core.messages.tool.ToolCallChunk)
...
At the moment, the output of the model will be in terms of AI Agents Force messages, so you will need to convert the output to the OpenAI format if you
need OpenAI format for the output as well.

The [convert_to_openai_messages](https://docs.aiagentsforce.com/api_reference/core/messages/aiagentsforce_core.messages.utils.convert_to_openai_messages.html) utility function can be used to convert from AI Agents Force messages to OpenAI format.
