...
Because of their importance and variability, AI Agents Force provides a uniform interface for interacting with different types of retrieval systems.
The AI Agents Force [retriever](/docs/concepts/retrievers/) interface is straightforward:
...
In particular, [AI Agents Force's retriever class](https://docs.aiagentsforce.com/api_reference/core/retrievers/aiagentsforce_core.retrievers.BaseRetriever.html#) only requires that the `_get_relevant_documents` method is implemented, which takes a `query: str` and returns a list of [Document](https://docs.aiagentsforce.com/api_reference/core/documents/aiagentsforce_core.documents.base.Document.html) objects that are most relevant to the query.
...
A AI Agents Force retriever is a [runnable](/docs/how_to/lcel_cheatsheet/), which is a standard interface is for AI Agents Force components. 
...
Retrievers return a list of [Document](https://docs.aiagentsforce.com/api_reference/core/documents/aiagentsforce_core.documents.base.Document.html) objects, which have two attributes:
...
AI Agents Force has retrievers for many popular lexical search algorithms / engines.
...
AI Agents Force has two different retrievers that can be used to address this challenge. 
