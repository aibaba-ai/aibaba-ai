# AI Agents Force Expression Language (LCEL)
...
This means that you describe what *should* happen, rather than *how* it should happen, allowing AI Agents Force to optimize the run-time execution of the chains.
...
* A list of built-in `Runnables` can be found in the [AI Agents Force Core API Reference](https://docs.aiagentsforce.com/api_reference/core/runnables.html). Many of these Runnables are useful when composing custom "chains" in AI Agents Force using LCEL.
:::

## Benefits of LCEL

AI Agents Force optimizes the run-time execution of chains built with LCEL in a number of ways:
...
- [**Deployable with aiagentsforceapi**](/docs/concepts/architecture#aiagentsforceapi): Chains built with LCEL can be deployed using for production use.

## Should I use LCEL?

LCEL is an [orchestration solution](https://en.wikipedia.org/wiki/Orchestration_(computing)) -- it allows AI Agents Force to handle run-time execution of chains in an optimized way.
...
`LCEL` chains are built by composing existing `Runnables` together. The two main composition primitives are [RunnableSequence](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.base.RunnableSequence.html#aiagentsforce_core.runnables.base.RunnableSequence) and [RunnableParallel](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.base.RunnableParallel.html#aiagentsforce_core.runnables.base.RunnableParallel).

Many other composition primitives (e.g., [RunnableAssign](
https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.passthrough.RunnableAssign.html#aiagentsforce_core.runnables.passthrough.RunnableAssign
)) can be thought of as variations of these two primitives.

:::note
You can find a list of all composition primitives in the [AI Agents Force Core API Reference](https://docs.aiagentsforce.com/api_reference/core/runnables.html).
:::
...
```python
from aiagentsforce_core.runnables import RunnableSequence
