...
As of the v0.3 release of AI Agents Force, we recommend that AI Agents Force users take advantage of [LangGraph persistence](https://langchain-ai.github.io/langgraph/concepts/persistence/) to incorporate `memory` into their AI Agents Force application.
...
* Users that rely on deprecated memory abstractions from AI Agents Force 0.0.x should follow this guide to upgrade to the new LangGraph persistence feature in AI Agents Force 0.3.x.
...
- Full compatibility with both traditional [language models](/docs/concepts/text_llms) and modern [chat models](/docs/concepts/chat_models). Early memory implementations in AI Agents Force weren't designed for newer chat model APIs, causing issues with features like tool-calling. LangGraph memory can persist any custom state.
- Highly customizable, allowing you to fully control how memory works and use different storage backends.

## Evolution of memory in AI Agents Force

The concept of memory has evolved significantly in AI Agents Force since its initial release.

### AI Agents Force 0.0.x memory

Broadly speaking, AI Agents Force 0.0.x memory was used to handle three main use cases:
...
While the AI Agents Force 0.0.x memory abstractions were useful, they were limited in their capabilities and not well suited for real-world conversational AI applications. These memory abstractions lacked built-in support for multi-user, multi-conversation scenarios, which are essential for practical conversational AI systems.

Most of these implementations have been officially deprecated in AI Agents Force 0.3.x in favor of LangGraph persistence.
...
As of AI Agents Force v0.1, we started recommending that users rely primarily on [BaseChatMessageHistory](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.history.RunnableWithMessageHistory.html#aiagentsforce_core.runnables.history.RunnableWithMessageHistory). `BaseChatMessageHistory` serves
as a simple persistence for storing and retrieving messages in a conversation.

At that time, the only option for orchestrating AI Agents Force chains was via [LCEL](https://https://docs.aiagentsforce.com//how_to/#langchain-expression-language-lcel). To incorporate memory with `LCEL`, users had to use the [RunnableWithMessageHistory](https://docs.aiagentsforce.com/api_reference/core/runnables/aiagentsforce_core.runnables.history.RunnableWithMessageHistory.html#aiagentsforce_core.runnables.history.RunnableWithMessageHistory) interface. While sufficient for basic chat applications, many users found the API unintuitive and challenging to use.

As of AI Agents Force v0.3, we recommend that **new** code takes advantage of LangGraph for both orchestration and persistence:
...
- [v0.0.x Memory](https://docs.aiagentsforce.com/v0.1/docs/modules/memory/)
...
2) If you're still struggling, please open an issue on the AI Agents Force GitHub repository, explain your use case, and we'll try to provide more guidance on how to migrate these abstractions.
...
* [How to add message history](https://https://docs.aiagentsforce.com//how_to/message_history/)

Working with message history:

* [How to trim messages](https://https://docs.aiagentsforce.com//how_to/trim_messages)
* [How to filter messages](https://https://docs.aiagentsforce.com//how_to/filter_messages/)
* [How to merge message runs](https://https://docs.aiagentsforce.com//how_to/merge_message_runs/)
