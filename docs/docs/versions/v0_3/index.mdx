# AI Agents Force v0.3

*Last updated: 09.16.24*

## What's changed

* All packages have been upgraded from Pydantic 1 to Pydantic 2 internally. Use of Pydantic 2 in user code is fully supported with all packages without the need for bridges like `aiagentsforce_core.pydantic_v1` or `pydantic.v1`.
...
- Added utilities for interacting with chat models: [universal model constructor](https://docs.aiagentsforce.com/v0.2/docs/how_to/chat_models_universal_init/), [rate limiter](https://docs.aiagentsforce.com/v0.2/docs/how_to/chat_model_rate_limiting/), [message utilities](https://docs.aiagentsforce.com/v0.2/docs/how_to/#messages),
- Added the ability to [dispatch custom events](https://docs.aiagentsforce.com/v0.2/docs/how_to/callbacks_custom_events/).
- Revamped integration docs and API reference. Read more [here](https://blog.langchain.dev/langchain-integration-docs-revamped/).
- Marked as deprecated a number of legacy chains and added migration guides for all of them. These are slated for removal in `langchain` 1.0.0. See the deprecated chains and associated [migration guides here](https://docs.aiagentsforce.com/v0.2/docs/versions/migrating_chains/).

## How to update your code

If you're using `langchain` / `langchain-community` / `langchain-core` 0.0 or 0.1, we recommend that you first [upgrade to 0.2](https://docs.aiagentsforce.com/v0.2/docs/versions/v0_2/).
...
The CLI will handle updating deprecated imports that were introduced in AI Agents Force 0.0.x and AI Agents Force 0.1, as
well as updating the `aiagentsforce_core.pydantic_v1` and `langchain.pydantic_v1` imports.
...
| aiagentsforceapi | 0.3.0  | >=0.3,&lt;0.4             |
...
- If your code depends on Pydantic aside from AI Agents Force, you will need to upgrade your pydantic version constraints to be `pydantic>=2,<3`.  See [Pydantic’s migration guide](https://docs.pydantic.dev/latest/migration/) for help migrating your non-AI Agents Force code to Pydantic v2 if you use pydantic v1.
- There are a number of side effects to AI Agents Force components caused by the internal switch from Pydantic v1 to v2. We have listed some of the common cases below together with the recommended solutions.

## Common issues when transitioning to Pydantic 2

### 1. Do not use the `aiagentsforce_core.pydantic_v1` namespace

Replace any usage of `aiagentsforce_core.pydantic_v1` or `langchain.pydantic_v1` with
...
### 2. Passing Pydantic objects to AI Agents Force APIs
...
### 3. Sub-classing AI Agents Force models

Any sub-classing from existing AI Agents Force models (e.g., `BaseTool`, `BaseChatModel`, `LLM`)
...
`pydantic.v1`, `aiagentsforce_core.pydantic_v1`, `langchain.pydantic_v1` should be replaced
...
# from aiagentsforce_core.pydantic_v1 import validator, Field
...
from aiagentsforce_core.pydantic_v1 import BaseTool
...
When sub-classing from AI Agents Force models, users may need to add relevant imports
...
The `langchain-cli` can help update deprecated AI Agents Force imports in your code automatically.

Please note that the `langchain-cli` only handles deprecated AI Agents Force imports and cannot
...
The `langchain-cli` will handle the `aiagentsforce_core.pydantic_v1` deprecation introduced in AI Agents Force 0.3 as well
...
After the first run, you’ll get: `from aiagentsforce_community.chat_models import ChatOpenAI`
