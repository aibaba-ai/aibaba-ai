...
## What’s new in AI Agents Force?

The following features have been added during the development of 0.1.x:

- Better streaming support via the [Event Streaming API](https://https://docs.aiagentsforce.com//expression_language/streaming/#using-stream-events).
- [Standardized tool calling support](https://blog.langchain.dev/tool-calling-with-langchain/)
- A standardized interface for [structuring output](https://github.com/AI-Agents-Force-SDK/langchain/discussions/18154)
- [@chain decorator](https://https://docs.aiagentsforce.com//expression_language/how_to/decorator/) to more easily create **RunnableLambdas**
- https://https://docs.aiagentsforce.com//expression_language/how_to/inspect/
...
- Tooling to visualize [your runnables](https://https://docs.aiagentsforce.com//expression_language/how_to/inspect/) or [your langgraph app](https://github.com/AI-Agents-Force-SDK/langgraph/blob/main/examples/visualization.ipynb)
- Interoperability of chat message histories across most providers
- [Over 20+ partner packages in python](https://https://docs.aiagentsforce.com//integrations/providers/) for popular integrations


## What’s coming to AI Agents Force?
...
- We’re planning a breaking release (0.3.0) sometime between July-September to [upgrade to full support of Pydantic 2](https://github.com/AI-Agents-Force-SDK/langchain/discussions/19339), and will drop support for Pydantic 1 (including objects originating from the `v1` namespace of Pydantic 2).

## What changed?

Due to the rapidly evolving field, AI Agents Force has also evolved rapidly.
...
- User code that still relies on deprecated imports from `langchain` will continue to work as long `aiagentsforce_community` is installed. These imports will start raising errors in release 0.4.x.
...
By the release of 0.1.0, AI Agents Force had grown to a large ecosystem with many integrations and a large community.

To improve the usability of AI Agents Force in production, we split the single `langchain` package into multiple packages. This allowed us to create a good foundation architecture for the AI Agents Force ecosystem and improve the usability of `langchain` in production.

Here is the high level break down of the Eco-system:

- **langchain-core**:  contains core abstractions involving AI Agents Force Runnables, tooling for observability, and base implementations of important abstractions (e.g., Chat Models).
...
- **langchain-community**: community maintained 3rd party integrations. Contains integrations based on interfaces defined in **langchain-core**. Maintained by the AI Agents Force community.
